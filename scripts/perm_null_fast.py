...
def fast_lopo_ridge(X, y, alpha):
    """Closed-form LOPO ridge, standardising on each training fold. Equivalent to
    lopo_ridge(X, y, [alpha]) but without sklearn object churn."""
    n = X.shape[0]
    pred = np.zeros(n)
    idx = np.arange(n)
    for i in range(n):
...
for f in csvs:
    ...
    pred = fast_lopo_ridge(X, y, 30.0)  # matched to permutation null
