...
def train_epoch(model, criterion, optimizer, train_data_loader, epoch):
    ...
    for i, batch in tqdm(enumerate(train_data_loader), total=len_train_data_loader):
        batch_loss = train_batch(model, optimizer, criterion, batch)
        running_loss += batch_loss
    print(f'Train loss: epoch {epoch + 1}: {running_loss / len_train_data_loader}')
...
def create_train_test_split(protein_dataset, random_state=42, split_mode='regular'):
    ...
    if split_mode == 'regular' or split_mode is None:
        train_idx, test_idx = train_test_split(range(len(protein_dataset)), test_size=config.training.split_size,
                                               random_state=random_state)
        test_idx, val_idx = train_test_split(test_idx, test_size=0.5, random_state=random_state)
        train_set = ProteinMutationDataset(protein_dataset.mutation_data.copy(), train_idx)
        test_set = ProteinMutationDataset(protein_dataset.mutation_data.copy(), test_idx)
...
def set_training_false_on_pretrained(model):
...
def train_single_protein(protein_name, model, optimizer, mutations_data, num_epochs):
    ...
    if config.training.freeze_pretrained:
        set_training_false_on_pretrained(model)
    for epoch in tqdm(range(num_epochs), desc="Epochs"):
        ...
        train_epoch(model, criterion, optimizer, train_data_loader, epoch)
        curr_test_loss = evaluate_single_protein(model, criterion, test_data_loader, epoch)
