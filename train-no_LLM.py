...
# Set wandb
if not CFG.debug:
    wandb.init(project="Thermodynamic+decoy",name = 'epoch 0 no LLM EMB')
if CFG.debug:
...
def main():
    ...
    d_params = data_params(num_workers =CFG.num_workers, batch_size=CFG.batch_size,cuda=CFG.cuda,
                           constraint=CFG.constraint, debug=CFG.debug,dataset='scn',LLM_EMB=False)
...
if __name__ == '__main__':
    if not CFG.debug:
        CFG.model_path = "./res/trianed_models-no_llm/"
